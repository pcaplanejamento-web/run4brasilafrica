...
- `CertificateModal` desenha o certificado num `<canvas>` **WYSIWYG** (`drawCertificate`) em
  ...
  **compartilhar** nativo (`navigator.share` com o arquivo). 100% no navegador, nada vai ao servidor.
  O **PDF** aceita **anotações `/Link`** (`PdfLink[]` em `buildImagePdf`): o **domínio do rodapé
  é clicável e abre o site** — o `drawCertificate` retorna o `domainRect` (px do canvas) e o
...
- Desenho **profissional** (`certificate.ts`): moldura dupla + cantos, marca/logo, título
  ...
  ADM (`domainBold`/`issuedBold`/`authBold`). Reusa `card.ts` (`ensureFonts`,
...
- **Verificação de autenticidade**: código no rodapé do certificado ⇄ botão **"Verificar
  certificado"** no **rodapé** e no **menu do header** (desktop e mobile) do site
  (`VerifyCertificateModal`, banner flutuante; gatilho reutilizável por `className`/`label`/`onOpen`
  — o `onOpen` fecha o menu mobile). O código
...
- **Fundo, cor do texto e escalas** (ADM): o fundo pode ser **padrão creme**, uma **cor sólida**
  ...
  (`textColor`) e as **escalas** por grupo — `scaleTitle`/`scaleName`/`scaleBody` (0.6–1.8) e
  **`scaleLogo` até 3.0 (300%)** (`scale(v, max)` no `certificate.ts`) — permitem aumentar título,
  nome, corpo e a **logo** em pontos específicos. O `drawBrand` **se adapta**: em logo-só a
  largura máxima cresce com a escala (`h/156` → até 72% do papel) sem estourar a moldura; no
...
- **Templates** (`drawCard` despacha por `state.template`): `foto` (foto/gradiente ao fundo +
  infos; a **marca — logo + nome do site — fica no RODAPÉ, na linha da categoria**, à direita, ao
  lado do "6KM"; o topo leva só o selo. Um lockup **grande** e bonito: `measureBrandLockup` mede/ajusta
  (reduz p/ caber em `~0.62·W`) e reserva a altura da linha, `drawBrandLockup` desenha **logo + filete
  divisor sutil + nome** (peso 800); sem logo → só o nome), `banner` (grade rótulo/valor — no claro a logo ganha **pílula escura** p/ contraste),

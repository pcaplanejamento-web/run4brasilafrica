...
/** Editor controlado do "Kit do atleta" (`KitSection`). `lotes` vem do global. */
export function KitEditor({
  ...
  value: KitSection;
  onChange: (next: KitSection) => void;
  lotes: Lote[];
  cloudinary?: Cloud;
}) {
  const kit = value;
  const set = (patch: Partial<KitSection>) => onChange({ ...kit, ...patch });
      ...
      <Card>
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          <div>
            ...
            <TextInput value={kit.title} onChange={(e) => set({ title: e.target.value })} />
          </div>
          <div>
            <FieldLabel>Subtítulo</FieldLabel>
            <TextInput value={kit.subtitle} onChange={(e) => set({ subtitle: e.target.value })} />
          </div>
          <div>
            <FieldLabel>Rótulo do botão do regulamento</FieldLabel>
            <TextInput
              value={kit.regulamentoLabel}
              onChange={(e) => set({ regulamentoLabel: e.target.value })}
            />
          </div>
        </div>
      </Card>
      ...
      <Card>
        ...
        <div className="mb-3 max-w-[280px]">
          ...
          <Select
            ...
            onChange={(e) => set({ regulamentoMode: e.target.value as "link" | "text" })}
          >
        ...
        {regText ? (
          <div>
            ...
            <TextArea
              ...
              onChange={(e) => set({ regulamentoText: e.target.value })}
            />
          </div>
        ) : (
          <div>
      ...
      <Card>
        ...
        <div className="mb-4 max-w-[320px]">
          ...
          <Select
            ...
            onChange={(e) => set({ kitMode: e.target.value as "single" | "perLote" })}
          >
        ...
        {perLote ? (
          lotes.length === 0 ? (
            <div className="text-[13px] text-adm-muted">
              Cadastre os lotes em &ldquo;Links &amp; inscrição&rdquo; para definir o kit de cada um.
            </div>
          ) : (

...
export interface CertificateData {
  ...
  showTeam?: boolean;
  domainBold?: boolean; // rodapé em negrito (opcional)
  issuedBold?: boolean;
  authBold?: boolean;
}

/** Retângulo (px do canvas) do domínio no rodapé — vira link clicável no PDF. */
export interface CertificateDrawResult {
  domainRect: { x: number; y: number; w: number; h: number } | null;
}
...
/** Escala segura (0.6–`max`, padrão 1.8), 1 quando ausente. */
function scale(v?: number, max = 1.8): number {
  return Math.max(0.6, Math.min(max, v || 1));
}
...
function drawBrand(ctx: CanvasRenderingContext2D, assets: CertificateAssets, data: CertificateData, cx: number, top: number, h: number, col: CertificatePalette) {
  ...
  if (hasLogo) {
    ...
    if (data.showEventName && name) {
      // Lockup horizontal: logo + nome. A logo **se adapta** limitando a largura
      // (até ~42% do papel) para sempre sobrar espaço ao nome — nada se sobrepõe.
      const maxLogoW = CERT_W * 0.42;
      const ldh = Math.min(dh, maxLogoW / ar);
      const ldw = ar * ldh;
      const gap = 52;
      const maxNameW = CERT_W * 0.6 - ldw;
      const namePx = fitFont(ctx, name, Math.round(ldh * 0.5), Math.round(ldh * 0.26), (px) => `800 ${px}px ${DISPLAY}`, maxNameW);
      ctx.font = `800 ${namePx}px ${DISPLAY}`;
      const nameW = ctx.measureText(name).width;
      const totalW = ldw + gap + nameW;
      const startX = cx - totalW / 2;
      ctx.drawImage(logo, startX, top, ldw, ldh);
      ctx.textAlign = "left";
      ctx.textBaseline = "middle";
      ctx.fillStyle = col.ink;
      ctx.fillText(name, startX + ldw + gap, top + ldh / 2 + namePx * 0.02);
      ctx.textBaseline = "alphabetic";
      return top + ldh;
    }
    // Só a logo, centrada. A largura máxima **cresce com a escala** (h/156) até 72%
    // do papel, permitindo aumentar a logo bem grande (até 300%) sem estourar a moldura.
    const s = dh / 156;
    const maxW = Math.min(CERT_W * 0.34 * s, CERT_W * 0.72);
...
/** Desenha o certificado inteiro. Retorna infos úteis (ex.: rect do domínio). */
export function drawCertificate(
  ...
  const sL = scale(data.scaleLogo, 3); // logo pode ir até 300%
  ctx.textBaseline = "alphabetic";
  ...
  // Gap abaixo da marca **se adapta**: com a logo grande, reduz o respiro para o
  // conjunto abaixo não ser empurrado contra as assinaturas.
  y = brandBottom + Math.max(40, 96 - Math.round((sL - 1) * 60));
  ...
  // ---- Rodapé (site + emissão + código) — cada trecho com peso próprio (ADM) ----
  // Desenha por segmento p/ permitir negrito individual e medir o domínio (link no PDF).
  const fSize = 32;
  const fy = H - m2 - 46;
  const sep = "   ·   ";
  const fontFor = (bold: boolean) => `${bold ? 700 : 400} ${fSize}px ${DISPLAY}`;
  const segs: { text: string; bold: boolean; domain?: boolean }[] = [];
  if (data.siteUrl) segs.push({ text: data.siteUrl, bold: !!data.domainBold, domain: true });
  if (data.issuedText) segs.push({ text: data.issuedText, bold: !!data.issuedBold });
  if (data.verifyCode) segs.push({ text: `Autenticação ${data.verifyCode}`, bold: !!data.authBold });

  let domainRect: CertificateDrawResult["domainRect"] = null;
  if (segs.length) {
    ctx.textAlign = "left";
    ctx.textBaseline = "alphabetic";
    ...
    segs.forEach((s, i) => {
      ...
      if (i < segs.length - 1) totalW += sepW;
    });
    let x = cx - totalW / 2;
    segs.forEach((s, i) => {
      ctx.font = fontFor(s.bold);
      ...
      ctx.fillText(s.text, x, fy);
      if (s.domain) domainRect = { x: x - 6, y: fy - fSize * 0.86, w: w + 12, h: fSize * 1.2 };
      x += w;
      if (i < segs.length - 1) {
        ctx.font = fontFor(false);
        ctx.fillStyle = col.soft;
        ctx.fillText(sep, x, fy);
        x += sepW;
      }
    });
  }
